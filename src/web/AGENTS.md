...
`src/web/` contains frontend product features, UI state, browser adapters, and
viewer/authoring presentation shells.
...
- Approval UI should display server proposal state and call approved actions; it
  must not mutate `DashboardDocument` locally.
- Viewer rendering must derive from `DashboardDocument`, execution results, and
  renderer materialization output rather than parallel frontend fixtures.

Forbidden dependencies:

- Do not import from `src/server/`.
- Do not import from another feature unless it is a deliberate shared module
  under `src/web/i18n/`, `src/web/api/`, `src/web/utils/`, `src/web/styles/`,
  `src/domain/`, or `src/contracts/`.
