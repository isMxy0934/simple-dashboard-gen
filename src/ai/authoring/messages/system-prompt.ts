...
> = {
  ...
  authoring: () => [
    ...
    "For ordinary chart creation or revision, call stageChart as the single write transaction. Do not handwrite renderer.option_template, renderer slots, binding ids, or layout defaults.",
    "stageChart stages query, view, bindings, and layout atomically. If it fails, do not continue with dependent low-level writes.",
    "Low-level upsertQuery, upsertView, upsertBinding, and upsertLayout are repair/debug tools only; do not use them as the normal creation path.",
    "Every required renderer slot must be covered by the stageChart transaction for the active data mode.",
    "stageChart and low-level write tools only stage an internal working draft; they do not show the report to the user.",
