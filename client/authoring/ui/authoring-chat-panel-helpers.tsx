...
import type { AiSuggestion } from "../../../ai/authoring/artifacts";
...
import { AUTHORING_AGENT_PATCH_APPROVAL_PART_TYPE } from "../../../ai/runtime/authoring-agent-client-parts";
import {
...
export type AgentMessagePart = AuthoringAgentMessage["parts"][number];
...
function renderAssistantMessageInOrder(input: {
  ...
  for (const part of message.parts) {
    ...
    if (part.type === AUTHORING_AGENT_PATCH_APPROVAL_PART_TYPE) {
      flushText();
      flushProcess();
      const data = (part as { data: AuthoringUiPatchApprovalPayload }).data;
      const draft =
...
export function getToolLabel(type: string, t: TranslateFn): string {
  const explicitKeys: Record<string, string> = {
    "tool-inspectContractState": "authoring.chat.toolLabels.inspectContractState",
    "tool-inspectDatasourceContext": "authoring.chat.toolLabels.inspectDatasourceContext",
    "tool-draftViews": "authoring.chat.toolLabels.draftViews",
    "tool-draftQueryDefs": "authoring.chat.toolLabels.draftQueryDefs",
    "tool-draftBindings": "authoring.chat.toolLabels.draftBindings",
    "tool-composePatch": "authoring.chat.toolLabels.composePatch",
    "tool-applyPatch": "authoring.chat.toolLabels.applyPatch",
    "tool-runRuntimeCheck": "authoring.chat.toolLabels.runRuntimeCheck",
  };
...
export function formatNextStepLabel(
  ...
  switch (nextStep) {
    case "layout":
      return t("authoring.chat.nextStep.layout");
    case "data":
      return t("authoring.chat.nextStep.data");
    case "repair":
      return t("authoring.chat.nextStep.repair");
    case "review":
      return t("authoring.chat.nextStep.review");
    default:
      return nextStep;
  }
}
...
export function buildFallbackWorkflowStages(
  ...
  > = {
    layout: {
      title: t("authoring.chat.workflowStage.layoutTitle"),
      description: t("authoring.chat.workflowStage.layoutDesc"),
    },
    data: {
      title: t("authoring.chat.workflowStage.dataTitle"),
      description: t("authoring.chat.workflowStage.dataDesc"),
    },
    repair: {
      title: t("authoring.chat.workflowStage.repairTitle"),
      description: t("authoring.chat.workflowStage.repairDesc"),
    },
    review: {
      title: t("authoring.chat.workflowStage.reviewTitle"),
      description: t("authoring.chat.workflowStage.reviewDesc"),
    },
  };
...
export function formatWorkflowModeLabel(
  ...
  switch (mode) {
    case "authoring":
      return t("authoring.chat.modeLabel.authoring");
    case "inspection":
      return t("authoring.chat.modeLabel.inspection");
    default:
      return mode;
  }
}
...
export function getRuntimeTimelineStatus(input: {
  ...
  runtimeSummaryOutput: AgentDraftOutput | null;
}): TaskTimelineStatus {
  if (
    input.agentError ||
  ...
  if (input.previewState === "loading" || input.activeStage === "repair") {
    return "active";
  }
...
export function getRuntimeTimelineText(
  ...
  if (input.activeStage === "repair" || input.activeStage === "review") {
...
export function formatWorkspaceSummaryText(
  ...
  switch (nextStep) {
    case "layout":
      return t("authoring.chat.workspaceSummary.layout");
    case "data":
      return t("authoring.chat.workspaceSummary.data");
    case "repair":
      return t("authoring.chat.workspaceSummary.repair");
    case "review":
      return t("authoring.chat.workspaceSummary.review");
    default:
...
export function formatPersistedTaskStatus(
  status: PersistedAuthoringTaskPayload["status"],
  t: TranslateFn,
) {
  switch (status) {
...
export function formatPersistedRuntimeStatus(
  status: PersistedAuthoringTaskPayload["runtimeStatus"],
  t: TranslateFn,
) {
  switch (status) {
    case "loading":
...
export function formatInterventionSummary(
  intervention: NonNullable<PersistedAuthoringTaskPayload["intervention"]>,
  t: TranslateFn,
) {
