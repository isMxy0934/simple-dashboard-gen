import type { AiSuggestion } from "../authoring/artifacts";
import type {
  AgentDraftOutput,
...
function deepCloneMessages(messages: AuthoringAgentMessage[]): AuthoringAgentMessage[] {
  return JSON.parse(JSON.stringify(messages)) as AuthoringAgentMessage[];
}
...
export function redactHeavyDashboardSnapshotsForTransport(
  ...
  for (let mi = 0; mi < next.length; mi++) {
    ...
    for (let pi = 0; pi < m.parts.length; pi++) {
      ...
      if (
        ...
        const out = p.output as AgentDraftOutput;
  ...
  for (let i = 0; i < composeSlots.length - 1; i++) {
    ...
    const part = next[mi].parts[pi] as { output: AgentDraftOutput };
