...
> = {
  identity: () => [
    ...
    "Prefer renderer formatting over changing SQL semantics. Use stageQuery only for SQL changes that preserve the existing QueryDef.output field names and types.",
    ...
    "Use compact GitHub-Flavored Markdown only: a short answer, optional bold section labels, bullets when useful, and at most one clear question.",
    "When a table is useful, emit a strict Markdown table: blank line before and after, one header row, one separator row, and each data row on its own line. Never inline multiple table rows into one paragraph.",
  ...
  inspect: () => [
    ...
    "You may answer directly or call read-only inspection tools.",
  ...
  authoring: () => [
    ...
    "To modify query logic without changing the output contract: (1) getQuery to read current SQL, (2) stageQuery with updated SQL that keeps the same output field names, (3) runCheck → composePatch.",
